...
void FastGICP<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  if (source_covs_.size() != input_->size()) {
    calculate_covariances(input_, *source_kdtree_, source_covs_, true);
  }
  if (target_covs_.size() != target_->size()) {
    calculate_covariances(target_, *target_kdtree_, target_covs_, false);
  }
...
template <typename PointSource, typename PointTarget>
double FastGICP<PointSource, PointTarget>::linearize_2D(const Eigen::Isometry3d& trans, Eigen::Matrix<double, 3, 3>* H, Eigen::Matrix<double, 3, 1>* b) {
  update_correspondences(trans);
...
#pragma omp parallel for num_threads(num_threads_) reduction(+ : sum_errors) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    ...
    Eigen::Matrix2d mahalanobis = mahalanobis_[i].block<2, 2>(0, 0);
    Eigen::Vector2d error_2D;
    error_2D.setZero();
    error_2D[0] = error[0];
    error_2D[1] = error[1];
    sum_errors += error_2D.transpose() * mahalanobis * error_2D;

    if (H == nullptr || b == nullptr) {
      continue;
    }

    Eigen::Matrix<double, 2, 3> dtdx0 = Eigen::Matrix<double, 2, 3>::Zero();
    dtdx0(0, 0) = transed_mean_A(1);
    dtdx0(1, 0) = -transed_mean_A(0);
    dtdx0.block<2, 2>(0, 1) = -Eigen::Matrix2d::Identity();

    Eigen::Matrix<double, 2, 3> jlossexp = dtdx0;
    Eigen::Matrix<double, 3, 3> Hi = jlossexp.transpose() * mahalanobis * jlossexp;
    Eigen::Matrix<double, 3, 1> bi = jlossexp.transpose() * mahalanobis * error_2D;
...
template <typename PointSource, typename PointTarget>
double FastGICP<PointSource, PointTarget>::compute_error_2D(const Eigen::Isometry3d& trans) {
  double sum_errors = 0.0;
...
#pragma omp parallel for num_threads(num_threads_) reduction(+ : sum_errors) schedule(guided, 8)
  for (int i = 0; i < input_->size(); i++) {
    ...
    Eigen::Matrix3d mahalanobis = mahalanobis_[i].block<3, 3>(0, 0);
    mahalanobis(2, 2) = 0.0;
    Eigen::Vector3d error_2D;
    error_2D.setZero();
    error_2D[0] = error[0];
    error_2D[1] = error[1];
    sum_errors += error_2D.transpose() * mahalanobis * error_2D;
...
#pragma omp parallel for num_threads(num_threads_) schedule(guided, 8)
  for (int i = 0; i < cloud->size(); i++) {
    ...
    // Arbitrarily set covariances to produce point-to-point.
    // covariances[i].setZero();
    // if (is_source) {
    //   continue;
    // } else {
    //   covariances[i].template block<3, 3>(0, 0) = Eigen::Matrix3d::Identity();
    //   continue;
    // }
