...
class TestICP(GtsamTestCase):
  ...
  def setUp(self):
    ...
    self.scan_c = np.array([[1], [1]])
    self.scan_d = np.array([[2], [1]])
    self.scan_f = np.array([[0, 20, 10], [0, 10, 20]])
    self.scan_g = np.array([[10, 30, 20], [10, 20, 30]])
    self.scan_h = np.array([[0.0, 1.0, 1.0], [0.0, 0.0, 0.5]])
    self.scan_i = np.array([[0, -1, 0], [0, 0, 1]])
